...
SELECT Agent_ID, CONCAT(Agent_F_Name,' ', Agent_L_Name) AS Agent_Name, Agent_Phone_Number FROM Agents;

--2. Create a list of customers by city
SELECT Cust_City, CONCAT(Cust_F_Name,' ', Cust_L_Name) AS Cust_Name FROM Customers 
...
SELECT Agent_ID, CONCAT(Agent_F_Name,' ', Agent_L_Name) AS Agent_Name, Agent_Date_Hired, DATEADD(day, 180, Agent_Date_Hired) as [Agent First Review Date] FROM Agents;
...
SELECT Customers.Cust_ID, CONCAT(Cust_F_Name,' ', Cust_L_Name) AS Cust_Name, Entertainers.Entertainer_ID, Entertainers.Entertainer_Stage_Name FROM Customers
...
SELECT Agents.Agent_ID, CONCAT(Agent_F_Name,' ', Agent_L_Name) AS Agent_Name, Engag_Number as [Contracts] FROM Agents
...
SELECT Customers.Cust_ID, CONCAT(Cust_F_Name,' ', Cust_L_Name) AS Cust_Name, Engagements.Engag_Number as [Booking] FROM Customers
...
SELECT Cust_Mus_Preferences.Cust_ID, CONCAT(Cust_F_Name,' ', Cust_L_Name) AS Cust_Name, Music_Style_Name FROM Customers
...
SELECT Agent_ID, CONCAT(Agent_F_Name,' ', Agent_L_Name) AS Agent_Name, [Agent_Street_ Address] , Agent_City, Agent_State, (Agent_Salary*Agent_Commission_Rate) AS [Commission above $1000] FROM Agents
